...
Thumb_Types decode (const ALL_Types data) {
    if (data.type.alu.instr.class_type.type_check == ALU_TYPE) {
        ...
        else if (data.type.ld_st.instr.class_type.opA == LD_ST_STB) {
        }
        else if (data.type.ld_st.instr.class_type.opA == LD_ST_LDB) {
        }
        else {
            cout << "NO TYPE FOUND: " << dec << data.type.ld_st.instr.class_type.opA << endl;
            return ERROR_TYPE;
        }
        return LD_ST;
    }
