...
int main(int argc, char** argv) {
  ...
  AoS           = (myNums *) malloc(sizeof(myNums));
  AoS->mydouble = (double *)malloc(NUMVALUES * sizeof(double));
  AoS->myfloat  = (float *)malloc(NUMVALUES * sizeof(float));
...
void init_nums (myNums *AoS) {
  ...
  for (unsigned int i = 0; i<NUMVALUES; ++i) {
    AoS->myfloat[i] = (rand_r(&fseed) / (float)RAND_MAX);
    AoS->mydouble[i] = (rand_r(&dseed) / (double)RAND_MAX);
  }
}
